...
def convert_to_percentage(data, row_list):
    # 转换成保留两位的百分比形式
    ...
    need_to_convert = [orders_wow, turnover, turnover_YoY, orders_YoY, orders_YoY_weekday, turnover_YoY_weekday]
    # 将output中的每一项数据都按照need_to_convert列表中的值，转换成保留两位的百分比形式
    for item in need_to_convert:
        data[item] = data[item].apply(lambda x: '{:.2%}'.format(x))
...
def filter_data(data, data_type):
    ...
    filters = {
        'increase': (data[orders_wow] > 1) | (data[turnover] > 1),
        'turnover drop': data[turnover] < -0.2,
        'order drop': data[orders_wow] < -0.2,
        'zero order': data[orders] == 0,
        'last time': data[last_time].apply(compare_times, args=('08:00:00',)) == 1,
    }

    try:
        # 根据data_type获取对应的过滤条件
        filter_condition = filters[data_type]
        output = data[filter_condition]
    except KeyError:
...
def get_informations(data, row_list, data_type, title):
    output, error = filter_data(data=data, data_type=data_type)

    output = output.drop_duplicates().sort_values(by='index')

    output = convert_to_percentage(data=output, row_list=row_list)
...
def output_to_excel(dataframe):
    # 将每一行都根据row_data中的name进行重新赋值，并转换成列表
    ...
    # 输出成excel
    merged_data = pd.concat([increase, turnover_drop, order_drop, zero_order, last_time_data], ignore_index=True)
